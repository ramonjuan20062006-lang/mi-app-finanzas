...
import { X, UserCog, Plus, Phone, Mail, Briefcase, Trash2, Search } from 'lucide-react'
...
export default function EmployeesModal() {
  ...
  const [form, setForm] = useState({ name: '', role: '', phone: '', email: '', salary: '', notes: '' })

  if (modal !== 'employees') return null
  ...
  const handleAdd = () => {
    ...
    setForm({ name: '', role: '', phone: '', email: '', salary: '', notes: '' })
    setShowForm(false)
  }
      ...
      <div className="w-full max-w-md bg-blue-50 rounded-t-3xl shadow-2xl max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="px-5 pt-5 pb-3 flex items-center justify-between border-b border-blue-100 flex-shrink-0">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-base font-bold text-blue-900">Empleados</h2>
              <p className="text-[11px] text-blue-400">{employees.length} empleados registrados</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
        ...
        <div className="px-4 pt-3 pb-2 flex-shrink-0">
          <div className="flex items-center gap-2 bg-white border border-blue-100 rounded-xl px-3 py-2">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar empleado..." className="flex-1 text-sm outline-none text-blue-900 placeholder:text-blue-300 bg-transparent" />
          </div>
        </div>
        ...
        {showForm && (
          <div className="mx-4 mb-3 bg-white rounded-2xl p-4 border border-blue-100 space-y-3 flex-shrink-0">
            <h3 className="text-sm font-bold text-blue-900">Nuevo empleado</h3>
            {[
              { key: 'name', placeholder: 'Nombre completo *' },
              { key: 'role', placeholder: 'Cargo / Rol' },
              { key: 'phone', placeholder: 'Teléfono' },
              { key: 'email', placeholder: 'Correo electrónico' },
              { key: 'salary', placeholder: 'Salario (USD)' },
            ].map(({ key, placeholder }) => (
              <input key={key} value={form[key as keyof typeof form]} onChange={e => setForm(f => ({ ...f, [key]: e.target.value }))}
                ...
                className="w-full px-3 py-2 bg-blue-50 border border-blue-100 rounded-xl text-sm text-blue-900 placeholder:text-blue-300 outline-none focus:ring-2 focus:ring-blue-200" />
            ))}
        ...
        <div className="flex-1 overflow-y-auto px-4 pb-6 space-y-2 scrollbar-hide">
          {filtered.length === 0 ? (
            <div className="flex flex-col items-center py-12">
              ...
              <p className="text-sm text-blue-400 text-center">{employees.length === 0 ? 'Aún no tienes empleados registrados.' : 'No se encontraron resultados.'}</p>
            </div>
          ) : (
            filtered.map(e => (
              <div key={e.id} className="bg-white rounded-2xl px-4 py-3 border border-blue-100 shadow-sm flex items-center gap-3">
                <div className="w-10 h-10 bg-blue-100 rounded-full flex items-center justify-center flex-shrink-0">
                  <span className="text-sm font-bold text-blue-600">{e.name[0].toUpperCase()}</span>
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-blue-900 truncate">{e.name}</p>
                  {e.role && <span className="flex items-center gap-1 text-[11px] text-blue-500"><Briefcase className="w-3 h-3" />{e.role}</span>}
                  <div className="flex items-center gap-3 mt-0.5">
                    {e.phone && <span className="flex items-center gap-1 text-[10px] text-blue-400"><Phone className="w-3 h-3" />{e.phone}</span>}
                    {e.salary && <span className="text-[10px] text-green-600 font-semibold">${e.salary}/mes</span>}
                  </div>
                  {e.email && <p className="flex items-center gap-1 text-[10px] text-blue-300 truncate"><Mail className="w-3 h-3" />{e.email}</p>}
                </div>
                <button onClick={() => handleDelete(e.id, e.name)} className="p-1.5 hover:bg-red-50 rounded-lg transition-colors">
