...
export default function BalanceScreen() {
  const { sales, expenses, loadSales, loadExpenses, setModal } = useStore()
  ...
  return (
    <div className="px-4 py-4 space-y-4">
          ...
          {days.map(d => (
            <button
              ...
              className={`px-3 py-1.5 rounded-xl text-xs font-semibold flex-shrink-0 transition-all ${
                d === selectedDate ? 'bg-blue-600 text-white shadow-md' : 'bg-white text-blue-500 border border-blue-100'
              }`}
            >
              {dateLabel(d)}
            </button>
      ...
      <div className="bg-white rounded-2xl p-5 shadow-sm border border-blue-100">
        ...
        <p className="text-3xl font-extrabold text-blue-900">${balance.toFixed(2)}</p>
      ...
      {isEmpty ? (
        ...
        <div className="space-y-2">
          {subTab === 'income'
            ? sales.map(s => (
              <div key={s.id} className="bg-white rounded-xl px-4 py-3 flex items-center justify-between border border-blue-100">
                <div>
                  <p className="text-sm font-medium text-blue-900">{s.description}</p>
                  <p className="text-[11px] text-blue-400">{s.date}</p>
                </div>
                <span className="text-sm font-bold text-green-600">+${Number(s.amount).toFixed(2)}</span>
              </div>
            ))
            : expenses.map(e => (
              <div key={e.id} className="bg-white rounded-xl px-4 py-3 flex items-center justify-between border border-blue-100">
                <div>
                  <p className="text-sm font-medium text-blue-900">{e.description}</p>
